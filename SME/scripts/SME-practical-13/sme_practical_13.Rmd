...
source(here("SME/functions/matched_functions.R"))
...
The survival package provides the `clogit()` function for conditional logistic regression, which is the standard method for analysing v for creating matched tables and calculating matched odds ratios.
...
```{r import}
# Import data
diabraz <- read_stata(here("SME","data", "diabraz.dta"))
...
mod1<-clogit(case ~ not_breastfed + strata(set), data = diabraz) |> 
...
diabraz2 <- read_stata(here("SME", "data", "diabraz2.dta"))
