...
whitehall <- read_stata(here("SME/data/whitehal.dta")) |> 
...
#Total deaths: 403 
#Total person-years: 27605.4
#Rate per 1000 person-years: 14.6
#95% CI: (13.24 , 16.1)
```
...
To estimate predicted rates per age group, we use avg_predictions from the marginaleffects package. We have to tell R which model we're using, which variables to group by, and use type = "response" to indicate we don't want any transformations.
...
model_age <- glm(all ~ agecat + offset(log(followup_years)), 
                 family = poisson, 
                 data = whitehall)
#model estimates how martality rates differ by age group, adjusting for different follow-up times

# Get predicted rates for each age group
# These are "marginal predictions" - the predicted rate within each age group
predictions_age <- avg_predictions(model_age, 
                                   variables = "agecat",
                                   type = "response")
#calculates the model-predicted mortality rate for each age category, adjusted for follow-up time (via the offset), expressed as actual rates (not log rates)

# Display as nicely printed table with rates per 1000 person-years
predictions_age |> 
  as_tibble() |> 
  mutate(rate_per_1000 = estimate * 1000,
         conf.low_1000 = conf.low * 1000,
         conf.high_1000 = conf.high * 1000) |> 
...
# Get predicted rates
predictions_chd <- avg_predictions(model_chd, 
                                   variables = "grade",
                                   type = "response")
