...
```{r survfit_overall}
# Calculate overall survival (regardless of predictor, what is the overall predictor)
# Alternatively, something = 1 represents the null hypothesis
...
```{r summary_time}
# Summary at 1 year (365.25 days), can change to 100 meaning what % of women survived after 100 days
...
```{r km_plot}
# Basic KM plot
#rx is treatment group, below: ovarian survival as explained by treatment group
...
```{r log_rank}
# Log-rank test comparing treatments NON-PARAMETRIC 
...
trinidad <- read_stata(here('/Users/finnwright16/Desktop/STuDy/MScEpi/SME/data/trinmlsh.DTA')) |> 
...
#Represents how long each participant was in the study before
#either dying or being censored

# Look at it
head(trinidad_surv, 20) #first 20 observations

# How many events?
table(trinidad$death) #How many died vs how many censored
```
...
fit_trinidad
#Fitting the model with trinidad_surv as outcome variable
#and ~1 as one single baseline survival function
#e.g. not separate curves for different treatments, sexes, or ages
...
```{r}
#--- KM plot stratified by smokers status (non/active smoker)
ggsurvplot(
...
The p-value indicates the strength of evidence against the null hypothesis of the same survival in each smoking group.
...
mort <- read_stata(here("SME/data/mortality.dta")) |> 
...
#Each participant was followed for X years, this 
#variable shows whether they died (1) or are 
#alive/censored (0)
...
```{r}
# Calculate rates per 1000 person-years by hypertension status SUMMARY TABLE
rate_table <- mort |> 
  filter(!is.na(hyper)) |> 
  group_by(hyper) |> 
  summarise(
...
#rateratio() expects a 2x2 matrix
...
# Rate ratio = 3.08
#The mortality rate among participants with hypertension is 3.08 times higher than among those without hypertension, after accounting for their different total person-years
```
...
#--- KM plot without CIs OR DELETE LABELS
ggsurvplot(
...
#The logrank test shows a statistically significant difference in survival between hypertensive and non-hypertensive participants
