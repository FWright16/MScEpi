...
here::here()
source(here("SME", "scripts","sme-2026-practical-6","mh_functions.R")) 
source(here("SME", "scripts","sme-2026-practical-6","or_function.R")) 
...
```{r import}
# Import the dataset
mwanza <- read_stata(here("SME","data", "mwanza.dta")) |> 
  mutate(across(where(is.labelled), as_factor)) #handling labelling from bringing in a Stata file

# Examine the structure
glimpse(mwanza)
summary(mwanza)
str(mwanza) #what variables type
```
...
#function(data, outcome, exposure)
# Odds Ratio
# ----------
# Odds Ratio: 2.416
# 95% CI: 1.678 - 3.478
...
- If the adjusted OR moves substantially toward 1.0, age explains part of the association (negative confounder)
- If the adjusted OR moves away from 1.0, age was masking part of the association (positive confounder)
...
- What is the M-H adjusted odds ratio for education?
 Odds Ratio: 2.416
 95% CI: 1.678 - 3.478
Crude OR is 2.416, after adjusting for age it becomes 2.331

- Does it provide evidence against the null hypothesis of no association after adjusting for age?
It provides evidence that age may be a slight negative confounder but the evidence is not strong

- Is there evidence of effect modification (do the stratum-specific ORs differ)?
Yes as the results are as follows: 15-24 (0.971), 25-34 (4.868), 35+ (2.396), these differ from each other
...
HIV status could confound an association between religion and education
...
Catholic (2.252), Muslim (2.022), Other (2.020), Protestant (1.394)
...
It provides evidence against the null hypothesis as the adjusted OR is above 1 with CIs that don't overlap 1

- Is there evidence of effect modification between religion and education?
There is no evidence of EM as the stratum-specific OR CIs all overlap each other, and a test for homogeneity produced a p-value of 0.7901
...
Yes the crude OR with missing values is 2.416, the crude OR restricted to complete cases is 2.311

- Compare the unadjusted and adjusted ORs (both on complete cases). Does npa confound the relationship?
The crude OR is 2.311 (1.597-3.345) and the adjusted OR is 2.417 (1.640-3.561), idicating positive confounding by number of sexual partners ever
