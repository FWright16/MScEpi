...
source(here("SME/scripts/mh_functions_updated.R"))
...
sum(is.na(mortality$systolic_grp))
```
...
```{r}
mod3 <- glm(died ~ systolic_grp_mid,
    data = mortality,
    family = binomial()) 
summary(mod3)
```
...
2b. Perform an LRT for effect modification between vimp and agegrp treated as continuous. Is there evidence that the association of visual impairment on death differs by age group, where age group is treated as continuous?
...
# Model without effect modification for LRT
model_noint <- glm(died ~ vimp + agegrp_cont,
                          data = mortality,
                          family = binomial())
tbl_regression(model_noint, exponentiate = TRUE)
