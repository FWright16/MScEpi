...
mhor <- function(data, outcome, exposure, strata = NULL) {
    ...
    for (stratum_level in strata_levels) {
        ...
        # M-H chi-squared components - FIX: Convert to numeric to avoid integer overflow
        m1 <- as.numeric(a + b)
        n1 <- as.numeric(a + c)
        n_float <- as.numeric(n)
        expected_a <- m1 * n1 / n_float
        mh_chisq_num <- mh_chisq_num + (a - expected_a)
        mh_chisq_denom <- mh_chisq_denom + (m1 * n1 * (n_float - m1) * (n_float - n1) / (n_float^2 * (n_float - 1)))
