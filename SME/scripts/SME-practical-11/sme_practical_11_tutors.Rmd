...
source(here("SME/scripts/SME-practical-11/mh_functions_updated.R"))
source(here("SME/scripts/sme-2026-practical-9/or_function.R"))
```
...
2. Fit logistic regression models to estimate the same association, without effect modification and with effect modification.

Note: in the model with effect modification, the interpretation of adjusted OR changes - it's a *stratum-specific* OR (in the baseline stratum of the other covariate)

```{r}
# Model without effect modification
mod1 <-glm(died ~ vimp + sex,
    data = mortality,
    family = binomial()) 
summary(mod1)
...
mod2 <- glm(died ~ vimp * sex, # the asterisk marks the effect modification (R creates a statistical interaction parameter)
    data = mortality,
    family = binomial()) 
summary(mod2)
...
model_int_sex <- glm(died ~ vimp * sex,
    data = mortality,
    family = binomial())
summary(model_int_sex)
```

What is the OR for visual impairment adjusted for sex? What is the OR for visual impairment *in men*? What is the OR for women (vs men) *in the visually unimpaired*? What is the statistical interaction term?
...
4. Fit a logistic regression model with effect modification between visual impairment and sex, and adjust for age
...
```{r}
# Import the dataset
mwanza <- read_dta(here("SME/data/mwanza.dta"))
...
Now use MH and logistic regression to assess for effect modification. Is there evidence of effect modification between age and education?
...
# Logistic regression without effect modification
